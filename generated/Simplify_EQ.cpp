ExprPtr Simplify_EQ(const EQ *expr, Simplify *simplifier) {
  ...
  if (const Select *a9 = expr->a->as<Select>()) {
    if (is_const_int(a9->a, 0)) {
      if (is_const_int(expr->b, 0)) {
        return (a9->cond || (a9->b == 0));
      }
    }
    if (is_const_v(a9->a)) {
      if (is_const_int(expr->b, 0)) {
        if (evaluate_predicate(fold((a9->a != 0)))) {
          return (!(a9->cond) && (a9->b == 0));
        }
      }
    }
    if (is_const_int(a9->b, 0)) {
      if (is_const_int(expr->b, 0)) {
        return (!(a9->cond) || (a9->a == 0));
      }
    }
    if (is_const_v(a9->b)) {
      if (is_const_int(expr->b, 0)) {
        if (evaluate_predicate(fold((a9->b != 0)))) {
          return (a9->cond && (a9->a == 0));
        }
      }
    }
  ...
  if (const Add *a19 = expr->a->as<Add>()) {
    if (const Select *a20 = a19->a->as<Select>()) {
      if (is_const_v(a20->a)) {
        if (is_const_v(a19->b)) {
          if (is_const_int(expr->b, 0)) {
            if (evaluate_predicate(fold(((a20->a + a19->b) == 0)))) {
              return (a20->cond || (a20->b == fold((0 - a19->b))));
            }
            if (evaluate_predicate(fold(((a20->a + a19->b) != 0)))) {
              return (!(a20->cond) && (a20->b == fold((0 - a19->b))));
            }
      ...
      if (is_const_v(a20->b)) {
        if (is_const_v(a19->b)) {
          if (is_const_int(expr->b, 0)) {
            if (evaluate_predicate(fold(((a20->b + a19->b) == 0)))) {
              return (!(a20->cond) || (a20->a == fold((0 - a19->b))));
            }
            if (evaluate_predicate(fold(((a20->b + a19->b) != 0)))) {
              return (a20->cond && (a20->a == fold((0 - a19->b))));
            }
