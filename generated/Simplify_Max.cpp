  ...
  if (const Select *a59 = expr->a.as<Select>()) {
    if (const Min *a60 = a59->true_value.as<Min>()) {
      if (equal(a60->a, expr->b)) {
        return min(select(a59->cond, a60->b, a59->false_value), a60->a);
        return select(a59->cond, a60->a, min(a59->false_value, a60->a));
      }
      if (equal(a60->b, expr->b)) {
        return min(select(a59->cond, a60->a, a59->false_value), a60->b);
        return select(a59->cond, a60->b, min(a59->false_value, a60->b));
      }
    }
    if (const Min *a64 = a59->false_value.as<Min>()) {
      if (equal(a64->a, expr->b)) {
        return min(select(a59->cond, a59->true_value, a64->b), a64->a);
        return select(a59->cond, min(a59->true_value, a64->a), a64->a);
      }
      if (equal(a64->b, expr->b)) {
        return min(select(a59->cond, a59->true_value, a64->a), a64->b);
        return select(a59->cond, min(a59->true_value, a64->b), a64->b);
      }
    }
    ...
    if (const Select *a225 = expr->b.as<Select>()) {
      if (equal(a59->cond, a225->cond)) {
        return select(a59->cond, min(a59->true_value, a225->true_value), min(a59->false_value, a225->false_value));
      }
    }
  }
  ...
  if (const Select *a182 = expr->b.as<Select>()) {
    ...
    if (const Min *a213 = a182->true_value.as<Min>()) {
      if (equal(expr->a, a213->b)) {
        return select(a182->cond, expr->a, min(expr->a, a182->false_value));
      }
    ...
    if (const Min *a221 = a182->false_value.as<Min>()) {
      if (equal(expr->a, a221->b)) {
        return select(a182->cond, min(expr->a, a182->true_value), expr->a);
      }
