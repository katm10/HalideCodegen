...
Expr Simplify_Sub(const Sub *expr, Simplify *simplifier) {
  ...
  if (is_const(expr->a)) {
    ...
    if (const Select *a92 = expr->b.as<Select>()) {
      if (is_const(a92->true_value)) {
        if (is_const(a92->false_value)) {
          return select(a92->cond, fold((expr->a - a92->true_value)), fold((expr->a - a92->false_value)));
        }
      }
    }
  }
    ...
    if (const Select *a90 = a14->a.as<Select>()) {
      if (const Select *a91 = expr->b.as<Select>()) {
        if (equal(a90->cond, a91->cond)) {
          return (select(a90->cond, (a90->true_value - a91->true_value), (a90->false_value - a91->false_value)) - a14->b);
        }
      }
    }
    if (is_const(a14->a)) {
    ...
    if (const Select *a78 = a17->a.as<Select>()) {
      if (const Select *a79 = expr->b.as<Select>()) {
        if (equal(a78->cond, a79->cond)) {
          return (select(a78->cond, (a78->true_value - a79->true_value), (a78->false_value - a79->false_value)) + a17->b);
        }
      }
    }
    if (const Select *a81 = a17->b.as<Select>()) {
      if (const Select *a82 = expr->b.as<Select>()) {
        if (equal(a81->cond, a82->cond)) {
          return (select(a81->cond, (a81->true_value - a82->true_value), (a81->false_value - a82->false_value)) + a17->a);
        }
      }
    }
    if (is_const(a17->b)) {
  ...
  if (const Select *a26 = expr->a.as<Select>()) {
    if (const Select *a27 = expr->b.as<Select>()) {
      if (equal(a26->cond, a27->cond)) {
        return select(a26->cond, (a26->true_value - a27->true_value), (a26->false_value - a27->false_value));
      }
    }
    if (equal(a26->true_value, expr->b)) {
      return select(a26->cond, 0, (a26->false_value - a26->true_value));
    }
    if (equal(a26->false_value, expr->b)) {
      return select(a26->cond, (a26->true_value - a26->false_value), 0);
    }
    if (const Add *a33 = a26->true_value.as<Add>()) {
      if (equal(a33->a, expr->b)) {
        return select(a26->cond, a33->b, (a26->false_value - a33->a));
      }
      if (equal(a33->b, expr->b)) {
        return select(a26->cond, a33->a, (a26->false_value - a33->b));
      }
      if (const Add *a42 = a33->b.as<Add>()) {
        if (equal(a42->b, expr->b)) {
          return select(a26->cond, (a33->a + a42->a), (a26->false_value - a42->b));
        }
        if (equal(a42->a, expr->b)) {
          return select(a26->cond, (a33->a + a42->b), (a26->false_value - a42->a));
        }
      }
      if (const Add *a48 = a33->a.as<Add>()) {
        if (equal(a48->a, expr->b)) {
          return select(a26->cond, (a33->b + a48->b), (a26->false_value - a48->a));
        }
        if (equal(a48->b, expr->b)) {
          return select(a26->cond, (a33->b + a48->a), (a26->false_value - a48->b));
        }
      }
      if (const Add *a54 = expr->b.as<Add>()) {
        if (equal(a33->a, a54->b)) {
          return (select(a26->cond, a33->b, (a26->false_value - a33->a)) - a54->a);
        }
        if (equal(a33->b, a54->b)) {
          return (select(a26->cond, a33->a, (a26->false_value - a33->b)) - a54->a);
        }
        if (equal(a33->a, a54->a)) {
          return (select(a26->cond, a33->b, (a26->false_value - a33->a)) - a54->b);
        }
        if (equal(a33->b, a54->a)) {
          return (select(a26->cond, a33->a, (a26->false_value - a33->b)) - a54->b);
        }
      }
    }
    if (const Add *a37 = a26->false_value.as<Add>()) {
      if (equal(a37->a, expr->b)) {
        return select(a26->cond, (a26->true_value - a37->a), a37->b);
      }
      if (equal(a37->b, expr->b)) {
        return select(a26->cond, (a26->true_value - a37->b), a37->a);
      }
    }
    if (const Add *a84 = expr->b.as<Add>()) {
      if (const Select *a85 = a84->a.as<Select>()) {
        if (equal(a26->cond, a85->cond)) {
          return (select(a26->cond, (a26->true_value - a85->true_value), (a26->false_value - a85->false_value)) - a84->b);
        }
      }
      if (const Select *a88 = a84->b.as<Select>()) {
        if (equal(a26->cond, a88->cond)) {
          return (select(a26->cond, (a26->true_value - a88->true_value), (a26->false_value - a88->false_value)) - a84->a);
        }
      }
    }
  }
  if (const Select *a30 = expr->b.as<Select>()) {
    if (equal(expr->a, a30->true_value)) {
      return select(a30->cond, 0, (expr->a - a30->false_value));
    }
    if (equal(expr->a, a30->false_value)) {
      return select(a30->cond, (expr->a - a30->true_value), 0);
    }
    if (const Add *a65 = a30->true_value.as<Add>()) {
      if (equal(expr->a, a65->a)) {
        return (0 - select(a30->cond, a65->b, (a30->false_value - expr->a)));
      }
      if (equal(expr->a, a65->b)) {
        return (0 - select(a30->cond, a65->a, (a30->false_value - expr->a)));
      }
    }
    if (const Add *a69 = a30->false_value.as<Add>()) {
      if (equal(expr->a, a69->a)) {
        return (0 - select(a30->cond, (a30->true_value - expr->a), a69->b));
      }
      if (equal(expr->a, a69->b)) {
        return (0 - select(a30->cond, (a30->true_value - expr->a), a69->a));
      }
    }
  }
