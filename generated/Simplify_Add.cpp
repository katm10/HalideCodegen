ExprPtr Simplify_Add(const Add *expr, Simplify *simplifier) {
  ...
  if (const Ramp *a2 = expr->a->as<Ramp>()) {
    ...
    if (const Ramp *a48 = a2->base->as<Ramp>()) {
      if (is_const_v(a48->lanes)) {
        if (is_const_v(a2->lanes)) {
          if (const Broadcast *a49 = expr->b->as<Broadcast>()) {
            if (is_const_v(a49->lanes)) {
  ...
  if (const Add *a10 = expr->a->as<Add>()) {
    if (const Broadcast *a11 = a10->b->as<Broadcast>()) {
      if (is_const_v(a11->lanes)) {
        if (const Broadcast *a12 = expr->b->as<Broadcast>()) {
          if (is_const_v(a12->lanes)) {
  ...
  if (const Sub *a22 = expr->a->as<Sub>()) {
    ...
    if (const Broadcast *a29 = a22->a->as<Broadcast>()) {
      if (is_const_v(a29->lanes)) {
        if (const Broadcast *a30 = expr->b->as<Broadcast>()) {
          if (is_const_v(a30->lanes)) {
  ...
  if (const Select *a34 = expr->a->as<Select>()) {
    if (const Select *a35 = expr->b->as<Select>()) {
      if (equal(a34->cond, a35->cond)) {
        return select(a34->cond, (a34->a + a35->a), (a34->b + a35->b));
      }
    }
    if (is_const_v(a34->a)) {
      if (is_const_v(a34->b)) {
        if (is_const_v(expr->b)) {
          return select(a34->cond, fold((a34->a + expr->b)), fold((a34->b + expr->b)));
        }
      }
      if (const Add *a40 = a34->b->as<Add>()) {
        if (is_const_v(a40->b)) {
          if (is_const_v(expr->b)) {
            return select(a34->cond, fold((a34->a + expr->b)), (a40->a + fold((a40->b + expr->b))));
          }
        }
      }
    }
    if (const Add *a38 = a34->a->as<Add>()) {
      if (is_const_v(a38->b)) {
        if (is_const_v(a34->b)) {
          if (is_const_v(expr->b)) {
            return select(a34->cond, (a38->a + fold((a38->b + expr->b))), fold((a34->b + expr->b)));
          }
        }
        if (const Add *a43 = a34->b->as<Add>()) {
          if (is_const_v(a43->b)) {
            if (is_const_v(expr->b)) {
    ...
    if (const Add *a51 = expr->b->as<Add>()) {
      if (const Select *a52 = a51->a->as<Select>()) {
        if (equal(a34->cond, a52->cond)) {
          return (select(a34->cond, (a34->a + a52->a), (a34->b + a52->b)) + a51->b);
        }
      }
      if (const Select *a55 = a51->b->as<Select>()) {
        if (equal(a34->cond, a55->cond)) {
          return (select(a34->cond, (a34->a + a55->a), (a34->b + a55->b)) + a51->a);
        }
      }
    }
    if (const Sub *a57 = expr->b->as<Sub>()) {
      if (const Select *a58 = a57->a->as<Select>()) {
        if (equal(a34->cond, a58->cond)) {
          return (select(a34->cond, (a34->a + a58->a), (a34->b + a58->b)) - a57->b);
        }
      }
      if (const Select *a61 = a57->b->as<Select>()) {
        if (equal(a34->cond, a61->cond)) {
          return (select(a34->cond, (a34->a - a61->a), (a34->b - a61->b)) + a57->a);
        }
      }
    }
    if (const Sub *a63 = a34->a->as<Sub>()) {
      if (is_const_v(a63->a)) {
        if (is_const_v(a34->b)) {
          if (is_const_v(expr->b)) {
            return select(a34->cond, (fold((a63->a + expr->b)) - a63->b), fold((a34->b + expr->b)));
            return (fold((a63->a + expr->b)) - select(a34->cond, a63->b, fold((a63->a - a34->b))));
          }
        }
      }
    }
      ...
      if (is_const_v(a65->b)) {
        if (is_const_v(expr->b)) {
          if (evaluate_predicate(fold(((a65->b + expr->b) == 0)))) {
            return select(a34->cond, (a34->a + expr->b), a65->a);
          }
        }
      }
    }
      ...
      if (const Add *a149 = a137->a->as<Add>()) {
        if (const Div *a150 = a149->b->as<Div>()) {
          if (const Add *a151 = a150->a->as<Add>()) {
            if (is_const_v(a151->b)) {
              if (is_const_v(a150->b)) {
                if (is_const_v(a137->b)) {
        ...
        if (const Div *a154 = a149->a->as<Div>()) {
          if (const Add *a155 = a154->a->as<Add>()) {
            if (is_const_v(a155->b)) {
              if (is_const_v(a154->b)) {
                if (is_const_v(a137->b)) {
        ...
        if (const Add *a380 = a375->a->as<Add>()) {
          ...
          if (const Div *a386 = a380->b->as<Div>()) {
            if (const Sub *a387 = a386->a->as<Sub>()) {
              if (is_const_v(a387->a)) {
                if (equal(a281->a, a387->b)) {
                  if (is_const_v(a386->b)) {
