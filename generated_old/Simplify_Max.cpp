...
Expr Simplify_Max(const Min *expr, Simplify *simplifier) {
  ...
  if (const Select *a59 = expr->a.as<Select>()) {
    if (const Min *a60 = a59->true_value.as<Min>()) {
      if (equal(a60->a, expr->b)) {
        return min(select(a59->condition, a60->b, a59->false_value), a60->a);
        return select(a59->condition, a60->a, min(a59->false_value, a60->a));
      }
      if (equal(a60->b, expr->b)) {
        return min(select(a59->condition, a60->a, a59->false_value), a60->b);
        return select(a59->condition, a60->b, min(a59->false_value, a60->b));
      }
    }
    if (const Min *a64 = a59->false_value.as<Min>()) {
      if (equal(a64->a, expr->b)) {
        return min(select(a59->condition, a59->true_value, a64->b), a64->a);
        return select(a59->condition, min(a59->true_value, a64->a), a64->a);
      }
      if (equal(a64->b, expr->b)) {
        return min(select(a59->condition, a59->true_value, a64->a), a64->b);
        return select(a59->condition, min(a59->true_value, a64->b), a64->b);
      }
    }
    if (const EQ *a187 = a59->condition.as<EQ>()) {
      if (is_const_v(a187->b)) {
        if (is_const_v(a59->true_value)) {
          if (equal(a187->a, a59->false_value)) {
            if (is_const_v(expr->b)) {
              if (evaluate_predicate(fold(((a187->b <= expr->b) && (a59->true_value <= expr->b))))) {
                return min(a187->a, expr->b);
              }
            }
            if (equal(a187->a, expr->b)) {
              if (evaluate_predicate(fold((a187->b < a59->true_value)))) {
                return select((a187->a == a187->b), a59->true_value, a187->a);
              }
              if (evaluate_predicate(fold((a59->true_value <= a187->b)))) {
                return a187->a;
              }
            }
          }
    ...
    if (const Select *a225 = expr->b.as<Select>()) {
      if (equal(a59->condition, a225->condition)) {
        return select(a59->condition, min(a59->true_value, a225->true_value), min(a59->false_value, a225->false_value));
      }
    }
  }
  ...
  if (const Select *a182 = expr->b.as<Select>()) {
    if (const EQ *a183 = a182->condition.as<EQ>()) {
      if (equal(expr->a, a183->a)) {
        if (is_const_v(a183->b)) {
          if (is_const_v(a182->true_value)) {
            if (equal(expr->a, a182->false_value)) {
              if (evaluate_predicate(fold((a183->b < a182->true_value)))) {
                return select((expr->a == a183->b), a182->true_value, expr->a);
              }
              if (evaluate_predicate(fold((a182->true_value <= a183->b)))) {
                return expr->a;
              }
            }
          }
    ...
    if (const Min *a213 = a182->true_value.as<Min>()) {
      if (equal(expr->a, a213->b)) {
        return select(a182->condition, expr->a, min(expr->a, a182->false_value));
      }
      if (equal(expr->a, a213->a)) {
        return select(a182->condition, expr->a, min(expr->a, a182->false_value));
      }
    }
    if (const Min *a221 = a182->false_value.as<Min>()) {
      if (equal(expr->a, a221->b)) {
        return select(a182->condition, min(expr->a, a182->true_value), expr->a);
      }
